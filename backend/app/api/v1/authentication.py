...
from app.model import (
        ...
        ForgotPasswordRequest,    # 忘记密码请求体模型
        ResetPasswordRequest,     # 重置密码请求体模型
    )
...
async def send_code(
    ...
    try:
        ...
        existing_user = register_manager.user_repository.get_user_by_email(request.email)

        if existing_user:
            # 邮箱已注册，返回409状态码
            raise HTTPException(status_code=409, detail="邮箱已经注册")
...
async def forgot_password(
    ...
    try:
        ...
        # 发送密码重置验证码
        success, message = register_manager.send_password_reset_code(request.email)
        ...
        return BaseResponse.fail(message=f"发送密码重置验证码失败: {str(e)}")
...
async def reset_password(
    ...
    try:
        ...
        # 重置密码
        success, message = register_manager.reset_password(
            email=request.email,
            verify_code=request.verify_code,
        ...
        return BaseResponse.fail(message=f"重置密码失败: {str(e)}")
